  ...
  class Act(argparse.Action):
    def __call__(self, parser, namespace, fname, option_string=None):
      ...
      if ext not in choices:
        ...
        parser.error("file "+fname+" doesn't end with one of {}{}".format(choices, option_string))
      else:
...
def input_read(): #Use argparse here.
  ...
  parser.add_argument('df_mut', metavar='<mut_data.csv>', help='.csv file containing the mutations. Takes absolute paths', action = CheckExt({'csv'}))
  parser.add_argument('df_loc', metavar='<exon_loc.txt>', help='.txt file containing the exon locations. Takes absolute paths', action = CheckExt({'txt'}))
...
def main():
  ...
  chr = 0
  B = 0
  make_backwards_dic = {'.':'.', 'A':'T', 'C':'G', 'T':'A', 'G':'C'}
  ...
  for exon in loc_data.splitlines():
    if header:
      if not chr:
        ...
        if not B%3 == 0:
          warnings.warn("Number of nucleotides is not divisible by 3. There might be something wrong with PDB data.")
        header = False
    else:
  ...
  for mutation in data.splitlines():
    ...
    if mutation_seq != '':
      if aa_mut != 'Del':
        ...
        post_mutation.append(make_backwards_dic[mutation_seq[-1]])
        pre_mutation.append(make_backwards_dic[base_num.group(0)[-1]])
      else:
        #base_num = re.search('(?<=.)\w+_\w+', mutation_seq)
        post_mutation.append(".")
        deleted = re.search('(?<=DEL)\w+', mutation_seq)
        backward = ''
        for letter in deleted.group(0):
          backward = backward + make_backwards_dic[letter]
        pre_mutation.append(backward)
